...
-----------------------------------------------
  ...
  const updatePendingPaymentsVisibility = async () => {
    try {
      ...
      setUsers((state) =>
        state?.map((_i) => (_i?.user_uuid === user_uuid ? user : _i))
      );
                        ...
                        <h1>
                          ...
                          <span>
                            (
                            {
                              [
                                ...new Set(
                                  orders_data?.map((_i) => _i?.counter_uuid)
                                ),
                              ]?.length
                            }
                            )
      ...
      {showSelect && (
        <div className="overlay">
          <div className="payment-summary-type">
            ...
            <div>
              <button onClick={() => createPaymentSummary(0)}>
                Pending payments only
              </button>
              <button onClick={() => createPaymentSummary(1)}>
                All payments
              </button>
            </div>
        ...
        <NotesPopup
          ...
          counterName={
            counter?.find(
              (i) =>
                i.counter_uuid ===
                selectedOrder?.[notesState?.index]?.counter_uuid
            )?.counter_title
          }
        />
      )}
...
const OrdersEdit = ({
  ...
  const postOrderData = async (deleteItems) => {
    ...
    for (let orderObject of dataArray) {
      ...
      const counterData = counter.find(
      ...
      let billingData = await Billing({
        ...
        items: data.item_details.map((a) => {
          ...
          const price =
            +counterData?.item_special_price?.find(
