...
const ProcessingOrders = () => {
        ...
        let sortedOrders = data.reduce(
          (result, order) =>
            !result.some((i) => i.counter_uuid === order.counter_uuid)
              ? result.concat(
                  data.filter((i) => i.counter_uuid === order.counter_uuid)
                )
              : result,
  ...
  const postOrderData = async (
    ...
    try {
      ...
      axios
        .post("/xpress/sendmessage/error", { error: stack })
        .catch(console.error);
    }
    setLoading(false);
  };
  ...
  return (
    <div>
      <nav
        ...
        style={{ top: "0px", padding: "10px", maxWidth: "500px" }}
      >
        <div
      ...
      {popupBarcode ? (
        <CheckingValues
          ...
          postOrderData={() =>
            postOrderData(selectedOrder ? [selectedOrder] : orders, false, true)
          }
